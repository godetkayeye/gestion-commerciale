...
# Script pour builder localement et préparer le déploiement
...
echo "📦 Étape 1/3: Génération de Prisma Client..."
npx prisma generate
echo "✅ Prisma Client généré"
echo ""

# 2. Build Next.js
echo "📦 Étape 2/3: Build Next.js..."
npm run build
echo "✅ Build Next.js terminé"
echo ""

# 3. Vérifier que Prisma Client est dans le build
echo "📦 Étape 3/3: Vérification de Prisma Client dans le build..."
...
if ls .next.tar.gz.part* 1> /dev/null 2>&1; then
    rm .next.tar.gz.part*
...
echo ""
echo "1. Pousser les parties vers Git (si < 100MB total):"
...
echo ""
echo "OU utiliser SCP directement (recommandé si > 100MB):"
echo "   scp .next.tar.gz ghost@72.61.109.17:/tmp/"
echo "   ssh ghost@72.61.109.17"
