...
interface RapportsCaisseBarClientProps {
  totalJour: number;
  countJour: number;
  totalSemaine: number;
  countSemaine: number;
  totalMois: number;
  countMois: number;
  facturesDetailJour: any[];
}

export default function RapportsCaisseBarClient({
  totalJour,
  countJour,
  totalSemaine,
  countSemaine,
  ...
  countMois,
  facturesDetailJour,
}: RapportsCaisseBarClientProps) {
  const [loading, setLoading] = useState<string | null>(null);

  const handleDownload = async (periode: string, format: string) => {
    setLoading(`${periode}-${format}`);
    try {
      const url = `/api/exports/rapport/caisse/bar?periode=${periode}&format=${format}`;
      ...
      if (res.ok) {
        ...
        a.download = `rapport-caisse-bar-${periode}-${new Date().toISOString().split("T")[0]}.${extension}`;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
          ...
          <div className="text-sm text-gray-600">{countJour} facture{countJour > 1 ? "s" : ""}</div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
          ...
          <div className="text-sm text-gray-600">{countSemaine} facture{countSemaine > 1 ? "s" : ""}</div>
          <div className="mt-4 flex gap-2">
            <button
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
          ...
          <div className="text-sm text-gray-600">{countMois} facture{countMois > 1 ? "s" : ""}</div>
          <div className="mt-4 flex gap-2">
            <button
      ...
      {/* Détails des factures d'aujourd'hui */}
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 bg-gray-50">
          <h2 className="text-lg font-semibold text-gray-900">Détails des factures (aujourd'hui)</h2>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100">
              {facturesDetailJour.length === 0 ? (
                ...
                facturesDetailJour.map((f) => (
                  <tr key={f.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">#{f.id}</td>
                    <td className="px-6 py-4 text-sm text-gray-700">
                      {f.commande_id ? `Commande #${f.commande_id}` : "-"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700">
                      {f.commande?.table?.nom || "-"}
                    </td>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900">
                      {Number(f.total ?? 0).toLocaleString("fr-FR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} FC
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700">
                      {f.date_facture ? new Date(f.date_facture).toLocaleString("fr-FR") : "-"}
                    </td>
                  </tr>
                ))
              )}
