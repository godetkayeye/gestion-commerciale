...
interface RapportsCaisseLocationClientProps {
  totalJour: number;
  countJour: number;
  totalSemaine: number;
  countSemaine: number;
...
export default function RapportsCaisseLocationClient({
  totalJour,
  countJour,
  totalSemaine,
  countSemaine,
  ...
  countMois,
  paiementsDetailJour,
}: RapportsCaisseLocationClientProps) {
  const [loading, setLoading] = useState<string | null>(null);

  const handleDownload = async (periode: string, format: string) => {
    setLoading(`${periode}-${format}`);
    try {
      const url = `/api/exports/rapport/caisse/location?periode=${periode}&format=${format}`;
      ...
      if (res.ok) {
        ...
        a.download = `rapport-caisse-location-${periode}-${new Date().toISOString().split("T")[0]}.${extension}`;
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Contrat</th>
                <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Bien</th>
                <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Locataire</th>
                <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Montant</th>
                <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase">Reste dû</th>
              ...
              {paiementsDetailJour.length === 0 ? (
                ...
                paiementsDetailJour.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-700">
                      {Number(p.reste_du ?? 0) > 0 ? (
                        <span className="text-red-600 font-medium">
                          {Number(p.reste_du).toLocaleString("fr-FR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} FC
                        </span>
                      ) : (
